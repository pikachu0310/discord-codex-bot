...
- `/start repository:<GitHubの名前/レポジトリ名>`:
  指定したリポジトリを準備して、専用の作業スレッドを作ります。以後、そのスレッドに書いた依頼は、リポジトリの作業コピー内で
  Codex が実行します。
  - 例: `/start repository:pikachu0310/discord-codex-bot`
- 作業スレッドに普通のメッセージを書く: その内容が Codex
  への依頼になります。修正、調査、テスト実行、説明依頼などをそのまま書けます。
- 作業スレッドに画像やファイルを添付する: 添付ファイルも Codex
  に渡されます。スクリーンショットを見せたい時や、対象ファイルを渡したい時に使えます。
- `!` から始まるメッセージを書く: Bot
  は反応しません。スレッド内でユーザー同士だけで会話したい時に使えます。
- `/stop`: そのスレッドで実行中の Codex
  を止めます。長い処理を中断したい時に使います。
- `/plan`:
  そのスレッドをプランモードにします。いきなり実装せず、まず方針や作業計画を出してほしい時に使います。
- `/active-threads`:
  まだ生きている作業スレッドを確認します。各アクティブスレッドから、コマンドを実行した人宛てに
  silent mention が送られます。
- `/close`: その作業スレッドを終了します。作業コピーを片付け、Discord
  のスレッドもクローズします。
